...
export async function POST(request: NextRequest) {
    const rl = await checkRateLimit(request, "uploads", 30, 60000);
    if (!rl.allowed) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
        return NextResponse.json({ error: "Invalid file type. Only PNG, JPEG, WebP, and SVG are allowed." }, { status: 400 });
    }
...
async function convertSvgUpload(request: NextRequest, formData: FormData, buffer: Buffer) {
    ...
    if (usage.kind === "guest" && usage.limitReached) {
        return errorResponse(429, "limit_reached", "You've used your 3 free conversions. Create a free account to keep converting.", undefined, request);
    }
