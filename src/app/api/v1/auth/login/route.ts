...
export async function POST(request: NextRequest) {
    const rl = await checkRateLimit(request, "auth:login", 10, 60000);
    if (!rl.allowed) {
    ...
    if (!user) {
        ...
        if (socialAccount) {
            ...
            return errorResponse(401, "social_login_required", "This email is linked to a social login (e.g. Google). Sign in with that option, or set a password using the \"Forgot password?\" link.", undefined, request);
        }
        await recordFailure(request, `login:${email}`);
    ...
    if (!user.isVerified) {
        return errorResponse(401, "email_not_verified", "Your email is not verified yet. Please verify your email first, then log in \u2014 check your inbox for the verification link.");
    }
    ...
    res.cookies.set(REFRESH_COOKIE_NAME, (payload.token as {
        refreshToken: string;
    }).refreshToken, {
        httpOnly: true,
