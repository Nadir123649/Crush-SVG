...
export async function POST(request: NextRequest) {
  const rl = await checkRateLimit(request, 'auth:change-password', 5, 60_000)
  if (!rl.allowed) {
  ...
  if (!user) return errorResponse(404, '', '', undefined, request)
  if (!user.password) {
    return errorResponse(400, '', '', undefined, request)
  }

  const isMatch = await verifyPassword(parsed.data.currentPassword, user.password)
  if (!isMatch) return errorResponse(401, '', '', undefined, request)
