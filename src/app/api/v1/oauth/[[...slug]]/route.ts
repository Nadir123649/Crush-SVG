...
export async function POST(request: NextRequest, { params }: {
    ...
    const rl = await checkRateLimit(request, `oauth:${provider}`, 10, 60000);
    if (!rl.allowed) {
    ...
    if (!parsed.success) {
        return errorResponse(400, "validation_error", Object.values(parsed.error.flatten().fieldErrors).flat()[0] ?? "Invalid input", undefined, request);
    }
    try {
        ...
        if (provider === "password" && token.email_verified !== true) {
            return errorResponse(403, "email_not_verified", "Email not verified. Please verify your email before logging in.", undefined, request);
        }
        ...
        const res = successResponse({
            ...
            token: tokenPair,
            sessionId: session._id.toString(),
        }, 200, rateLimitHeaders(rl), request);
