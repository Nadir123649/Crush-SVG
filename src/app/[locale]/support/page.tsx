...
export default async function SupportPage({
  ...
  const contactSchema = {
    ...
    "@type": "ContactPage",
    name: t("metaTitle"),
    url: `${SITE_URL}${locale === "en" ? "/support" : `/${locale}/support`}`,
    description: t("metaDesc"),
  };
  ...
  return (
    <div className="w-full flex flex-col items-center md:pb-[60px] min-h-[60vh]">
      ...
      <Hero
        badge={t("badge")}
        title={<>{t("title").split(" ")[0]} <span className="bg-gradient-to-r from-brand-primary to-brand-secondary text-transparent bg-clip-text">{t("title").split(" ").slice(1).join(" ") || "Center"}</span></>}
        subtitle={<>{t("subtitle")} <Link href="/team" className="text-brand-primary hover:underline font-medium">our team</Link> directly.</>}
