 ...
 *
 * Uses `jose` directly instead of the Firebase Admin SDK: it avoids pulling in
 * the SDK's ESM-only dependency graph (`jwks-rsa` → `jose`), which crashes with
 * ERR_REQUIRE_ESM on runtimes without `require(esm)` support (e.g. some Vercel
 * function runtimes).
 */
