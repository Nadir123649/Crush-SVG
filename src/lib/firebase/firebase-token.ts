...
export function mapTokenPayload(payload: JWTPayload): DecodedIdToken {
    ...
    const firebaseClaim = payload.firebase && typeof payload.firebase === "object"
        ? (payload.firebase as Record<string, unknown>)
        : {};
    const signInProvider = typeof firebaseClaim.sign_in_provider === "string" ? firebaseClaim.sign_in_provider : "";
    return {
        uid: sub,
