...
function rebuildTag(tag: string): string {
    ...
    if (name.toLowerCase() === "svg") {
        const hasNamespace = kept.some((a) => a.startsWith("xmlns=") && a.includes(SVG_NAMESPACE));
        const hasXlink = kept.some((a) => a.startsWith("xmlns:xlink="));
        if (!hasNamespace)
            kept.unshift(`xmlns="${SVG_NAMESPACE}"`);
        if (!hasXlink)
...
function removeElements(svg: string, tagName: string): string {
    ...
    do {
        previous = sanitized;
        sanitized = sanitized.replace(new RegExp(`<${escaped}\\b[^>]*>[\\s\\S]*?<\\/${escaped}\\s*>`, "gi"), "");
