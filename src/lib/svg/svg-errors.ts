...
export function classifySvgError(error: unknown): SvgErrorInfo {
    ...
    if (/corrupt header|xml parse error|expected end tag|mismatched tag|unexpected eof|invalid tag|no such element/.test(low)) {
        return {
            status: 422,
            code: "invalid_svg",
            message: "Your SVG markup is malformed \u2014 check for unclosed tags or mismatched quotes and try again.",
        };
    }
    if (/unable to load font|fontconfig|no fonts found|unknown font/.test(low)) {
        return {
            status: 422,
            code: "svg_font_error",
            message: "Your SVG uses a font that isn't available on the server \u2014 convert text to paths (outline the font) or use a standard web font.",
        };
    }
