...
/**
 * Single source of truth for SVG intrinsic size, shared by the client (UI
 * labels) and the server (conversion sizing). Only bare pixel lengths are
 * accepted — `%`, `em`, `cm` etc. are ignored. When both width/height
 * attributes are present they win; otherwise the viewBox is used for both
 * axes so client and server always agree on the aspect ratio.
 */
...
/**
 * Computes the effective output size for a conversion. An exact width+height
 * produces a padded `contain` canvas; a single dimension scales proportionally
 * from the SVG's intrinsic size; otherwise the intrinsic size is scaled.
 * Any side exceeding MAX_OUTPUT_SIZE throws OutputTooLargeError — output is
