...
export async function revokeSession(sessionId: string, userId: SessionUserId): Promise<boolean> {
    const result = await Session.updateOne({ _id: sessionId, userId }, { $set: { status: "revoked" } });
    return result.modifiedCount > 0;
}
export async function revokeAllSessions(userId: SessionUserId, status: "logged_out" | "revoked"): Promise<void> {
...
export async function rotateSession(sessionId: string, expectedVersion: number, userId: SessionUserId): Promise<{
    rotated: boolean;
    currentVersion: number;
    remember: boolean;
}> {
    const updated = await Session.findOneAndUpdate({ _id: sessionId, userId, tokenVersion: expectedVersion, status: "active" }, { $inc: { tokenVersion: 1 }, $set: { rotatedAt: new Date(), lastSeenAt: new Date() } }, { new: true });
    if (updated) {
        return {
            rotated: true,
