...
export function Button({ children, className = "", variant = "solid", ...props }: ButtonProps) {
  ...
  return (
    <button
      className={`bg-gradient-to-r from-[#D94A1E] to-[#FF9A3D] text-white rounded-[12px] px-[10px] hover:opacity-90 transition-opacity active:scale-[0.98] ${baseTextStyles} ${className}`}
      {...props}
    >
      {children}
    </button>
