        ...
        <div className="w-full h-[626.23px] bg-[#FFFFFF] border border-dashed border-[#8F8F8F] rounded-[24px] flex justify-center px-[40px] pt-[20px] pb-[20px] gap-[30px]">
          <div className="w-[536.9px] flex flex-col">
            <div className="flex items-center justify-between mb-[12px]">
              ...
              {usage && (
                <span className="font-body font-normal text-[13px] text-[#64748B]">
                  {usage.isUnlimited
            ...
            <textarea
              ...
              spellCheck={false}
              aria-label="SVG code"
              className="w-full h-[302px] rounded-[16px] border border-[#8F8F8F] p-[24px] bg-white resize-none outline-none focus:border-brand-primary transition-colors font-body font-normal text-[16px] leading-[18.67px] text-[#D2D2D2] whitespace-pre-wrap overflow-auto"
            />
            ...
            <div
              ...
              className={`w-full h-[167px] rounded-[16px] border border-[#8F8F8F] mt-[16px] flex flex-col items-center justify-center gap-[10px] p-[40px] cursor-pointer hover:bg-gray-50 transition-colors ${dragOver ? "bg-gray-50 border-brand-primary" : ""}`}
            >
              <Image src={IMAGES.drag} alt="" width={64} height={64} className="object-contain" />
          ...
          <div className="w-[536.9px] flex flex-col">
            ...
            <div className="w-full h-[302px] rounded-[16px] border border-[#8F8F8F] flex items-center justify-center relative overflow-hidden bg-gray-50/30">
              {previewUrl ? (
                ...
                <Image src={IMAGES.uploadImage} alt="" width={100} height={100} className="object-contain" />
              )}
            </div>
            ...
            <div className="w-full mt-[20px]" ref={dropdownRef}>
              <div className="flex gap-[20px] w-full">
                <div className="flex flex-col flex-1 gap-[8px] relative">
                  ...
                  {openDropdown === "width" && (
                    <div role="listbox" className="absolute top-[90px] left-0 w-full max-h-[200px] overflow-y-auto bg-white border border-[#8F8F8F] rounded-[12px] shadow-lg z-10 py-[8px]">
              ...
              <div className="flex justify-center mt-[16px] gap-[12px]">
                {limitReached && status !== "authed" ? (
                  ...
                  <>
                    <Button className="h-[42px] px-[32px] rounded-[12px] gap-[8px]" onClick={handleConvert} disabled={converting}>
                      {converting ? (
                        <span className="flex items-center gap-[8px]">
                          <span className="w-[16px] h-[16px] rounded-full border-[2px] border-white/40 border-t-white animate-spin" />
                          Converting…
                        </span>
                      ) : (
                        <>
                          <Image src={IMAGES.exportIcon} alt="" width={18} height={18} className="brightness-0 invert" />
                          Convert to PNG
                        </>
                      )}
                    </Button>
                    <Button
                      variant="outline"
                      className="h-[42px] px-[32px] rounded-[12px] gap-[8px]"
                      onClick={handleDownload}
                      disabled={!result?.data}
                    >
                      <Image src={IMAGES.downloadImage} alt="" width={18} height={18} className="object-contain" />
                      Download
                    </Button>
                  </>
                )}
              ...
              {result && result.size !== undefined && (
                <p className="text-center font-body font-normal text-[13px] text-[#64748B] mt-[10px]">
